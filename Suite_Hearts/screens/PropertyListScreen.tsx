...
export default function PropertyListScreen({
  ...
  const {
    properties: datafinitiProperties,
    loading,
    error,
    selectedPropertyId,
    ...
    try {
      ...
      if (data) {
        ...
        if (currentUser) {
          ...
          const otherList = listings.filter(
            (l) => l.ownerId !== currentUser.id
          );
  ...
  const handlePropertyPress = (property: Property) => {
    ...
    navigation.navigate("ListingDetail", {
      listing: { ...property, source: "external" },
    });
  };
  ...
  const likedListingsData = allListings.filter((listing) =>
      ...
      <View style={styles.container}>
        ...
        <ScrollView
          ...
          contentContainerStyle={[
            ...
            likedListingsData.length === 0 && styles.emptyScrollContent,
          ]}
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    navigation.navigate("ListingDetail", {
                      listing: {
                        ...listing,
                        source: listing.ownerId ? "user" : "external",
                      },
                    });
                  }}
                  activeOpacity={0.7}
  ...
  return (
    <View style={[styles.container, { paddingTop: 50 }]}>
      ...
      >
        ...
        {combinedUserListings.filter(
          (l) => !searchQuery.trim() || filteredListings.includes(l)
        ).length > 0 && (
          <>
            <Text style={styles.sectionTitle}>All Listings</Text>
            {combinedUserListings
              .filter(
                (l) => !searchQuery.trim() || filteredListings.includes(l)
              )
              .map((listing) => {
                const isMyListing =
                  currentUser && listing.ownerId === currentUser.id;
                const photo = getListingPhoto(listing);
                return (
                  <TouchableOpacity
                    key={listing.id}
                  ...
                  >
                    ...
                    <View
                      style={[
                        styles.propertyInfo,
                        isMyListing && styles.myListingInfo,
                      ]}
                    >
                      <Text
                        style={
                          isMyListing ? styles.myListingPrice : styles.price
                        }
                      >
                        ${listing.price.toLocaleString()}
                      </Text>
                      <Text
                        style={
                          isMyListing ? styles.myListingAddress : styles.address
                        }
                      >
                        {listing.address}
                      </Text>
        ...
        {externalListings.filter(
          ...
          <>
            {(myListings.filter(
              ...
              <Text style={styles.sectionTitle}>External Listings</Text>
            )}
            {externalListings
              .filter(
                (l) => !searchQuery.trim() || filteredListings.includes(l)
